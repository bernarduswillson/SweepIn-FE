  ...
  return (
    <div className="relative h-fit flex flex-col justify-end pt-[50px]">
      <div className="mb-7 transition ease-in-out duration-500">
        <h1
          ref={ref}
          className={`text-white text-5xl pt-10 poppins-extrabold transition ease-in-out duration-500 ${isInView ? 'opacity-100' : 'opacity-0 translate-y-5'}`}
        >
          {title}
        </h1>
        <h2
          ref={ref}
          className={`text-white text-md poppins-medium transition ease-in-out duration-500 delay-200 ${isInView ? 'opacity-100' : 'opacity-0 translate-y-5'}`}
        >
      ...
      <div className="absolute top-0 -right-[20px] md:hidden">
        ...
        <Image
          src={title === 'Presensi' ? Calendar : TablerReport}
          alt="Calendar"
          width={220}
          height={220}
        />
