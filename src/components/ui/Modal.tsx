...
const Modal = (props: ModalProps): JSX.Element => {
  const { title, msg, img, type, confirmText, onConfirm, cancelText, onClose } = props;
  ...
  return (
    <div className={`fixed z-[50] inset-0 flex items-center justify-center bg-black bg-opacity-60 transition-opacity ease-in-out duration-300 ${isOpen ? 'opacity-100' : 'opacity-0'}`}>
      ...
      <div className={`flex flex-col items-center bg-white max-w-[500px] w-11/12 p-5 pt-10 rounded-md transition-transform ease-in-out duration-300 transform scale-${isOpen ? '100' : '0'}`}>
