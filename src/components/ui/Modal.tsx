...
const Modal = (props: ModalProps): JSX.Element => {
  const { title, msg, img, type, confirmText, onConfirm, cancelText, onClose, isOpen } = props;
        ...
        >
          ...
          >
            ...
            <div className='flex justify-center items-center gap-5'>
              <button onClick={onConfirm} className={`poppins-medium px-7 py-2 text-white rounded-3xl border-solid border-[3px] button-animation ${type === 'info' ? 'bg-green_main border-green_main hover:bg-green_dark hover:border-green_dark' : 'bg-red_main border-red_main hover:bg-red_dark hover:border-red_dark'}`}>
                {confirmText}
              </button>
              {
                cancelText && 
