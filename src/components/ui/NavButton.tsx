  ...
  return (
    <button className="w-fit relative flex py-2 ">
      <div className="z-[10] flex">
        <div
          className={`px-5 poppins-medium transition-all ease-in-out duration-500 ${active === 'Presensi' ? 'text-blue_main' : 'text-white'}`}
          onClick={() => handleToggleClick('Presensi')}
        >
          Presensi
        </div>
        <div
          className={`px-5 poppins-medium transition-all ease-in-out duration-500 ${active === 'Laporan' ? 'text-blue_main' : 'text-white'}`}
      ...
      <div
        className={`bg-white absolute h-full w-[50%] rounded-md top-0 transition-all ease-in-out duration-500 ${active === 'Presensi' ? 'left-0' : 'left-1/2'}`}
      />
    </button>
  )
}
