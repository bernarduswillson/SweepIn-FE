...
const SearchBar = (props: SearchBarProps): JSX.Element => {
  ...
  return (
    <div className="w-full flex gap-2">
      {/* Start date input */}
      <button
        className={`w-[40%] flex justify-between items-center py-1 px-2 pl-3 border-grey border-2 rounded-xl poppins-medium ${monthRange.start ? 'text-black' : 'text-grey_text'}`}
        ...
        {showCalendarFrom && (
          <>
            ...
            <div className="fixed top-1/2 left-1/2 flex flex-col py-5 px-5 items-center gap-5 transform -translate-x-1/2 -translate-y-1/2 z-20 bg-white rounded-md border shadow">
              ...
              <button
                className="w-[80px]"
                onClick={(e) => handleOkeButtonClick(e)}
              >
      ...
      {/* End date input */}
      <button
        className={`w-[40%] flex justify-between items-center py-1 px-2 pl-3 border-2 rounded-xl poppins-medium ${!monthRange.start ? 'text-grey_text border-grey' : isRangeValid ? 'text-black border-grey' : 'text-red_main border-red_main'}`}
          ...
          <>
            ...
            <div className="fixed top-1/2 left-1/2 flex flex-col py-5 px-5 items-center gap-5 transform -translate-x-1/2 -translate-y-1/2 z-20 bg-white rounded-md border shadow">
              ...
              <div className="w-full h-fit flex flex-col items-center gap-2">
                {!isRangeValid && (
                  <span className="text-sm text-red_main poppins-medium">
