          ...
          <div className="w-full max-w-[641px] flex justify-center flex-grow py-6 bg-white rounded-t-[26px]">
            <div className="w-11/12 h-fit flex flex-col">
              ...
              <h3 className="text-black text-xl poppins-medium">
                {data.endLog
                  ? dateTimeRange2String(
                      new Date(data.startLog.date),
                      new Date(data.endLog.date)
                    )
                  : dateTime2String(new Date(data.startLog.date))}
              </h3>
              ...
              <div className="my-2 rounded-xl overflow-hidden">
                <iframe
                  src={`https://maps.google.com/maps?q=${data.startLog.latitude},${data.startLog.longitude}&z=15&output=embed`}
                  width="100%"
                  height="200"
                  style={{ border: 0 }}
                  allowFullScreen
              ...
              {data.endLog && (
                <>
                  ...
                  <div className="my-2 rounded-xl overflow-hidden">
                    <iframe
                      src={`https://maps.google.com/maps?q=${data.endLog.latitude},${data.endLog.longitude}&z=15&output=embed`}
                      width="100%"
                      height="200"
                      style={{ border: 0 }}
                      allowFullScreen
